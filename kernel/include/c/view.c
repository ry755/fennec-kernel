...
void view_render(framebuffer_t *target, view_t *view_first) {
    ...
    do {
        ...
        view_render(view_current->framebuffer, view_current->next_child);
...
void view_copy(framebuffer_t *target, view_t *source) {
    ...
    for (int16_t y = source->y; y < (source->y + source->height); y++) {
        if (y >= target->height || y < 0) continue;
        for (int16_t x = source->x; x < (source->x + source->width); x++) {
            if (x >= target->width || x < 0) continue;
            target->memory[y * target->width + x] = source->framebuffer->memory[source_y * source->framebuffer->width + source_x];
            source_x++;
        }
        source_x = 0;
        source_y++;
